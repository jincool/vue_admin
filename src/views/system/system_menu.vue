        ...
        methods: {
            ...
            getMenu() {
                return new Promise((resolve, reject) => {
                    this.$api.get('?f=menu&c=Menu&a=getMenu').then(res => {
                        if (res.status === 200) {
                            this.menu = res.data;
            ...
            //获取子菜单
            getSubMenu(id){
                this.$api.post('?f=menu&c=Menu&a=getSubMenu',{'id':id}).then(res => {
                    if (res.status === 200) {
            ...
            deleteMenu(type, id){
                this.deleteBox('?f=menu&c=Menu&a=deleteMenu',{'id':id,'type':type}).then(status=>{
                    if (status===1) {
                        this.getMenu()
                    }
                });
            ...
            editSubMenu(type, id,inputValue){
                this.mesEditBox('请修改菜单名', '?f=menu&c=Menu&a=editMenu', {'id':id,'type':type},inputValue).then(status=>{
                    if (status===1) {
            ...
            // 增加主菜单
            addMenu() {
                this.mesEditBox('请添加主菜单', '?f=menu&c=Menu&a=addMenu', {}).then(status=>{
                    if (status===1) {
                        this.getMenu()
                    }
                });
            ...
            addSubMenu(){
                ...
                this.$api.post('?f=menu&c=Menu&a=addSubMenu', params).then(res => {
            ...
            refreshSort(type){
                ...
                this.$api.post('?f=menu&c=Menu&a=refreshSort', params).then(res => {
                    if (res.data.status === 1) {
                        this.$message({
                            type: 'info',
                            message: '操作成功'
